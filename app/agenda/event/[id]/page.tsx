...
export default async function EventPage({ params }: EventPageProps) {
  ...
  let item = allItems.find((i: any) => i.id === eventId) as AgendaItem | undefined;
...
const returnUrl = `/agenda/${parentDay.label.toLowerCase().includes('workshop') ? 'workshop' : 'sessioni'}/${parentDay.slug}`;

  return (
    <div className="container mx-auto py-10">
      <div className="max-w-3xl mx-auto">
      <Link href={returnUrl} className="text-blue-600 mb-6 inline-block">
        ← Torna all&apos;agenda
      </Link>
        ...
        {item.presenters && (
          <p className="text-gray-600 mb-4"><strong>🎤 Relatori:</strong> {item.presenters}</p>
        )}

        {item.description && (
          <p className="text-gray-800 whitespace-pre-line mb-6">{item.description}</p>
        )}

        {item.tags && (
          <div className="mt-6">
            <strong className="text-gray-500 block mb-1">Tag:</strong>
        ...
        {item.speakers && item.speakers.length > 0 && (
          <>
            <h2 className="text-2xl font-bold mt-16 mb-6">Relatori &apos;incontro</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {item.speakers.map((speaker) => (
                <AgendaSpeakerCard key={speaker.id} speaker={speaker} />
              ))}
            </div>
          </>
        )}
