...
export default async function WorkshopDayPage({ params }: WorkshopDayPageProps) {
  ...
  const day = data.data.find(
    (day: any) => day.slug === params.id && day.label.toLowerCase().includes('workshop')
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {timeSlot.items.map((item, itemIndex) => (
                  <div
                    ...
                    style={{
                      borderLeftWidth: '5px',
                      borderLeftColor: item.color || '#808080',
                    }}
                  >
                    {item.id ? (
